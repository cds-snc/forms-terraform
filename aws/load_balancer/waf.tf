      ...
      not_statement {
        statement {
          regex_pattern_set_reference_statement {
            arn = aws_wafv2_regex_pattern_set.valid_app_uri_path_regex_array.arn
            field_to_match {
              uri_path {}
            }
            text_transformation {
...
resource "aws_waf_regex_pattern_set" "valid_app_uri_path_regex_array" {
  ...
  regex_pattern_strings = ["^\\/(?:en|fr)?\\/?(?:(admin|id|api|auth|signup|myforms|unsupported-browser|terms-of-use|404)(?:\\/[\\w-]+))?\\/?$", "^\\/(?:en|fr)?\\/?(?:(form-builder|sla|unlock-publishing|terms-and-conditions)(?:\\/[\\w-]+))?\\/?$", "^\\/(?:en|fr)?\\/?(?:(static|_next|img|favicon\\.ico)(?:\\/[\\w-]+))?\\/?$", "^\\/(?:en|fr)?\\/?(?:\\/[\\w-]+)?\\/?$"]
}