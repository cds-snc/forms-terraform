...
while :; do
  ...
  RECEIPT_HANDLES=()
  while IFS='' read -r line; do RECEIPT_HANDLES+=("$line"); done < <(echo "$RAW_MESSAGE" | python3 -c "$PYTHON_CODE")

  echo "Running Reliability Queue Lambda"
  echo '{"Records":'"$RAW_MESSAGE"'}' | sam local invoke -t ./local_development/template.yml --event - "AuditLogs"
