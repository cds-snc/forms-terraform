...
async function getSubmission(message) {
  ...
  const DBParams = {
    ...
    ProjectExpression: "SubmissionID,FormID,SendReceipt,FormData,FormSubmissionLanguage,SubmissionDateTime",
  };
...
async function saveToVault(submissionID, formResponse, formID, submissionDateTime) {
  const db = new DynamoDBClient({ region: REGION, endpoint: process.env.AWS_SAM_LOCAL ? "http://host.docker.internal:4566": undefined });
