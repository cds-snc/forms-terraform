...
const createSQLString = (formID) => {
  // TODO: originally thought about putting conditional check here
  // BUT -- just get the tem."isPublished" value and return it
...
const parseQueryResponse = (records) => {
  if (records.length === 1) {
    ...
    if (!process.env.AWS_SAM_LOCAL) {
      ...
      return { name, emailAddress: record[0].stringValue, isPublished: record[3].booleanValue };
    } else {
      const name = record.name !== "" ? record.name : `${record.jsonConfig.titleEn} - ${record.jsonConfig.titleFr}`;
      return { name, emailAddress: record.email, isPublished: record.isPublished };
    }
  } else {
    return null;
  }
