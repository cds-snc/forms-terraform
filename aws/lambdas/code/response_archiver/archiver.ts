...
async function archiveConfirmedFormResponses(
...
async function retrieveConfirmedFormResponses(dynamodb: DynamoDBClient): Promise<FormResponse[]> {
  try {
    ...
    throw new Error(
      `Failed to retrieve confirmed form responses. Reason: ${(error as Error).message}.`
    );
  }
}
...
async function archiveFormResponseAttachments(
  ...
  try {
    ...
    throw new Error(
      `Failed to copy form response attachments from Vault to Archiving S3 buckets. Reason: ${
...
async function archiveFormResponse(
  s3Client: S3Client,
  formId: string,
  submissionId: string,
  responsesAsJson: string
...
async function deleteFormResponsesAttachmentsFromVault(
  s3Client: S3Client,
...
async function deleteFormResponsesFromVault(
  dynamodb: DynamoDBClient,
  formResponses: FormResponse[]
) {
  /**
